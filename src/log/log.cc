...
DEF_bool(also_log_to_local, false,
         "#0 if true, also log to local file when write-cb is set");
...
public:
  ...
  enum { b_min = 8, b_sec = b_min + 3, b_ms = b_sec + 3, b_max = b_ms + 3 };
...
LevelLogger::LevelLogger()
    : _log_event(true, false), _fs(new fastream(1 << 20)), _path(256), _stop(0),
      _check_failed(false), _ex_handler(0) {
...
void LevelLogger::init() {
  ...
  if (!fs::exists(_config->log_dir))
    fs::mkdir(_config->log_dir, true);
...
void LevelLogger::set_write_cb(
    const std::function<void(const void *, size_t)> &cb) {
  _write_cb = cb;
}

void LevelLogger::set_single_write_cb(
    const std::function<void(const void *, size_t)> &cb) {
  _single_write_cb = cb;
...
void LevelLogger::write(fastream *fs) {
  ...
  if (FLG_cout)
    fwrite(fs->data(), 1, fs->size(), stderr);
}
    ...
    if (!need_create_new_file) {
      if (!_old_paths.empty()) {
        ...
        if (fs::fsize(_path) >= FLG_max_log_file_size ||
            this->get_day_from_path(path) != _day) {
          fs::rename(_path, path); // rename xx.log to xx_0808_15_30_08.123.log
          if (FLG_log_compress)
            this->compress_rotated_log(path);
...
void LevelLogger::on_failure(int sig) {
  ...
  if (!_stmp.empty()) {
    if (_file)
      _file.write(_stmp.data(), _stmp.size());
    write_to_stderr(_stmp.data(), _stmp.size());
  }

  if (_stack_trace)
    _stack_trace->dump_stack(&_file,
                             _check_failed ? 7 : (sig == SIGABRT ? 4 : 3));
  if (_file) {
    _file.write('\n');
    _file.close();
  }
...
int LevelLogger::on_exception(PEXCEPTION_POINTERS p) {
  ...
  if (_stack_trace)
    _stack_trace->dump_stack(&_file, 6);
  if (_file) {
    _file.write('\n');
    _file.close();
  }
...
void on_signal(int sig) { level_logger()->on_signal(sig); }

void on_failure(int sig) { level_logger()->on_failure(sig); }
...
LevelLogSaver::LevelLogSaver(const char *file, int len, unsigned int line,
  ...
  (_s << ' ' << co::thread_id() << ' ').append(file, len)
      << ':' << line << ']' << ' ';
}
...
void exit() { xx::level_logger()->stop(); }
