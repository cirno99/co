...
public:
  ...
  struct Task {
    Task(F &&f, int p, int c) : fun(f), period(p), count(c) {}
  ...
  TaskedImpl() : _stop(0), _mtx(), _ev(), _t(&TaskedImpl::loop, this) {}
...
// if @daily is false, run f() only once, otherwise run f() every day at
// hour:minute:second
...
Tasked::Tasked() { _p = new TaskedImpl(); }
...
void Tasked::stop() { ((TaskedImpl *)_p)->stop(); }
