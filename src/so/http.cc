...
DEF_uint32(http_max_body_size, 8 << 20,
           "#2 max size of http body, default: 8M");
...
DEF_uint32(http_conn_idle_sec, 180,
           "#2 http server may close the connection if no data was recieved "
           "for n seconds");
...
Client::Client(const char *serv_url) {
  ...
  if (strncmp(serv_url, "https://", 8) == 0 ||
      strncmp(serv_url, "http://", 7) == 0) {
    s.append(serv_url);
  } else {
...
void *Client::easy_handle() const { return _ctx->easy; }
...
int Client::response_code() const {
  long code = 0;
  const CURLcode r =
      curl_easy_getinfo(_ctx->easy, CURLINFO_RESPONSE_CODE, &code);
...
const char *Client::header() const { return _ctx->header.c_str(); }

const char *Client::body() const { return _ctx->body.data(); }

size_t Client::body_size() const { return _ctx->body.size(); }
...
size_t easy_header_cb(char *p, size_t size, size_t nmemb, void *userp) {
  ...
  if (r != CURLE_OK || code == 100)
    return n;

  if (!h.empty()) {
    if (n > 2)
      ctx->add_header((uint32)h.size());
    h.append(p, n);
  } else {
...
int parse_http_req(fastring *buf, http_req_t *req) {
  ...
  { /* parse body size */
    ...
    if (*v == '\0' || *v == '0') {
      ...
      int n = atoi(v);
      if (n >= 0) {
        if ((uint32)n > FLG_http_max_body_size)
          return 413;
        req->body_size = n;
        return 0;
      }
...
ServerImpl::ServerImpl()
    : _buffer([]() { return (void *)new fastring(4096); },
              [](void *p) { delete (fastring *)p; }),
      _conn_num(0) {}

void ServerImpl::start(const char *ip, int port, const char *key,
                       const char *ca) {
...
void ServerImpl::on_connection(tcp::Connection conn) {
  ...
  while (true) {
    ...
    recv_beg:
      if (!buf) {
        ...
        if (r < 0) {
          if (!co::timeout())
            goto recv_err;
          if (_conn_num > FLG_http_max_idle_conn)
            goto idle_err;
          goto recv_beg;
        }
      ...
      while ((pos = buf->find("\r\n\r\n")) == buf->npos) {
        if (buf->size() > FLG_http_max_header_size)
          goto header_too_long_err;
        buf->reserve(buf->size() + 1024);
        r = conn.recv((void *)(buf->data() + buf->size()),
      ...
      // parse http header
      if (preq == 0)
        preq = (http_req_t *)calloc(1, sizeof(http_req_t));
      if (pres == 0)
        pres = (http_res_t *)calloc(1, sizeof(http_res_t));
      ...
      if (preq->body_size > 0 ||
          strcmp(preq->header("Transfer-Encoding"), "chunked") != 0) {
        total_len = pos + 4 + req.body_size();
        if (preq->body_size > 0 && buf->size() < total_len) {
          buf->reserve(total_len);
          r = conn.recvn((void *)(buf->data() + buf->size()),
         ...
         *   难逃如来佛掌中。       you can't escape from the palm of the
         * Buddha. 六字真言显威灵，          The six syllable Mantra put in the
         * code, 从此bug去无踪。               nowhere to find a bug any more.
         */
        ...
        while (true) { /* loop for recving chunked data */
          while ((x = s.find("\r\n")) == s.npos) {
            ...
            s.reserve(s.size() + 32);
            r = conn.recv((void *)(s.data() + s.size()), 32,
                          FLG_http_recv_timeout);
            if (r == 0)
              goto recv_zero_err;
            if (r < 0)
          ...
          if (n > 0) {
            ...
            if (o < n) {
              ...
              r = conn.recvn((void *)(buf->data() + buf->size()),
                             (int)(n - o + 2), FLG_http_recv_timeout);
              if (r == 0)
                goto recv_zero_err;
              if (r < 0)
                goto recv_err;
    ...
    { /* handle the http request */
      ...
      if (!s.empty())
        pres->add_header("Connection", s.c_str());

      if (preq->version == kHTTP10) {
        if (s.empty() || s.tolower() != "keep-alive")
          need_close = true;
      } else {
        if (!s.empty() && s == "close")
          need_close = true;
...
void easy(const char *root_dir, const char *ip, int port, const char *key,
          const char *ca) {
  http::Server serv;
  std::vector<LruMap<fastring, std::pair<fastring, int64>>> contents(
      co::scheduler_num());
