...
public:
  ServerImpl()
      : _conn_num(0), _buffer([]() { return (void *)new fastring(4096); },
  ...
  void add_userpass(const char *s) {
    ...
    if (x.is_object()) {
      for (auto it = x.begin(); it != x.end(); ++it) {
        ...
        auto pass = it.value();
        if (!pass.is_string()) {
          ELOG << "add userpass failed, user: " << user
...
Server::~Server() { delete (ServerImpl *)_p; }

void Server::add_service(Service *s) { ((ServerImpl *)_p)->add_service(s); }
...
void ServerImpl::on_connection(tcp::Connection conn) {
  if (!_userpass.empty() && !this->auth(&conn)) {
    ELOG << "auth failed, reset connection " << conn.socket()
         << " 3 seconds later..";
    conn.reset(3000);
    return;
  }
  ...
  while (true) {
    ...
    recv_beg:
      ...
      if (unlikely(r < 0)) {
        if (!co::timeout())
          goto recv_err;
        if (_conn_num > FLG_rpc_max_idle_conn)
          goto idle_err;
      ...
      if (unlikely(header.magic != kMagic))
        goto magic_err;

      len = ntoh32(header.len);
      if (unlikely(len > FLG_rpc_max_msg_size))
        goto msg_too_long_err;
...
bool ServerImpl::check_userpass(const char *user, const char *pass,
                                const char *nonce) {
...
bool ServerImpl::auth(tcp::Connection *conn) {
  ...
  do {
    ...
    if (!user.is_string() || !pass.is_string() ||
        !this->check_userpass(user.get_string(), pass.get_string(),
                              res["nonce"].get_string())) {
      res.clear();
      res.add_member("method", "auth");
      res.add_member("err", 401);
...
bool ClientImpl::connect() {
  if (!_tcp_cli.connect(FLG_rpc_conn_timeout))
    return false;
...
void ClientImpl::call(const Json &req, Json &res) {
  ...
  if (!_tcp_cli.connected() && !this->connect())
    return;
