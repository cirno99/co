...
bool Event::wait(uint32 ms) const { return ((EventImpl *)(_p + 2))->wait(ms); }

void Event::signal() const { ((EventImpl *)(_p + 2))->signal(); }
...
void WaitGroup::add(uint32 n) const { atomic_add(_p + 1, n); }

void WaitGroup::done() const {
  CHECK_GT(*(_p + 1), (uint32)0);
  if (atomic_dec(_p + 1) == 0)
    ((EventImpl *)(_p + 2))->signal();
}

void WaitGroup::wait() const { ((EventImpl *)(_p + 2))->wait((uint32)-1); }
...
void Mutex::lock() const { ((MutexImpl *)(_p + 2))->lock(); }

void Mutex::unlock() const { ((MutexImpl *)(_p + 2))->unlock(); }

bool Mutex::try_lock() const { return ((MutexImpl *)(_p + 2))->try_lock(); }

class PoolImpl {
public:
  typedef std::vector<void *> V;

  PoolImpl() : _pools(co::scheduler_num()), _maxcap((size_t)-1) {}

  PoolImpl(std::function<void *()> &&ccb, std::function<void(void *)> &&dcb,
           size_t cap)
      : _pools(co::scheduler_num()), _maxcap(cap), _ccb(std::move(ccb)),
        _dcb(std::move(dcb)) {}
...
void *Pool::pop() const { return ((PoolImpl *)(_p + 2))->pop(); }

void Pool::push(void *p) const { ((PoolImpl *)(_p + 2))->push(p); }

void Pool::clear() const { ((PoolImpl *)(_p + 2))->clear(); }

size_t Pool::size() const { return ((PoolImpl *)(_p + 2))->size(); }
...
public:
  PipeImpl(uint32 buf_size, uint32 blk_size, uint32 ms)
      : _buf_size(buf_size), _blk_size(blk_size), _rx(0), _wx(0), _ms(ms),
        _full(false) {
...
void Pipe::read(void *p) const { ((PipeImpl *)(_p + 2))->read(p); }

void Pipe::write(const void *p) const { ((PipeImpl *)(_p + 2))->write(p); }
