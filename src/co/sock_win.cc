...
sock_t socket(int domain, int type, int protocol) {
  sock_t fd =
      CO_RAW_API(WSASocketW)(domain, type, protocol, 0, 0, WSA_FLAG_OVERLAPPED);
...
inline int get_address_family(sock_t fd) {
  ...
  CHECK(r == 0 && info.iAddressFamily > 0)
      << "get address family failed, fd: " << fd;
...
sock_t accept(sock_t fd, void *addr, int *addrlen) {
  ...
  if (fd == (sock_t)-1)
    return CO_RAW_API(accept)(fd, (sockaddr *)addr, addrlen);
  ...
  if (r == FALSE) {
    if (co::error() != ERROR_IO_PENDING)
      goto err;
    ev.wait();
  }

  // https://docs.microsoft.com/en-us/windows/win32/api/mswsock/nf-mswsock-acceptex
  r = co::setsockopt(connfd, SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT, &fd,
                     sizeof(fd));
  if (r != 0) {
...
int recvfrom(sock_t fd, void *buf, int n, void *addr, int *addrlen, int ms) {
  ...
  if (N > 0) {
    ...
    *(int *)s = sizeof(SOCKADDR_STORAGE);
    r = CO_RAW_API(WSARecvFrom)(fd, &ev->buf, 1, &ev->n, &ev->flags,
                                (sockaddr *)(s + 8), (int *)s, &ev->ol, 0);
  } else {
    r = CO_RAW_API(WSARecvFrom)(fd, &ev->buf, 1, &ev->n, &ev->flags, 0, 0,
                                &ev->ol, 0);
  }
...
int sendto(sock_t fd, const void *buf, int n, const void *addr, int addrlen,
  ...
  do {
    int r = CO_RAW_API(WSASendTo)(fd, &ev->buf, 1, &ev->n, 0,
                                  (const sockaddr *)addr, addrlen, &ev->ol, 0);
    if (r == 0) {
      if (!can_skip_iocp_on_success)
        ev.wait();
    } else if (co::error() == WSA_IO_PENDING) {
  ...
  const char *strerror(int e) {
    ...
    if (it != _p->pos.end()) {
      ...
      FormatMessageA(
          ...
          0, e, MAKELANGID(LANG_ENGLISH /*LANG_NEUTRAL*/, SUBLANG_DEFAULT),
          (LPSTR)&s, 0, 0);
...
const char *strerror(int err) {
  if (err == ETIMEDOUT || err == WSAETIMEDOUT)
    return "Timed out.";
  static co::Error e;
