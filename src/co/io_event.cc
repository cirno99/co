...
bool IoEvent::wait(uint32 ms) {
  ...
  // If fd is a non-blocking TCP socket, we'll post an I/O operation to IOCP
  // using WSARecv or WSASend. Since _info->buf is empty, no data will be
  // transfered, but we should know that the socket is readable or writable when
  // the IOCP completes.
  if (_nb_tcp != 0) {
    if (_nb_tcp == nb_tcp_recv) {
      int r = CO_RAW_API(WSARecv)(_fd, &_info->buf, 1, &_info->n, &_info->flags,
                                  &_info->ol, 0);
      if (r == 0 && can_skip_iocp_on_success)
        return true;
      if (r == -1 && co::error() != WSA_IO_PENDING)
        return false;
    } else {
      int r =
          CO_RAW_API(WSASend)(_fd, &_info->buf, 1, &_info->n, 0, &_info->ol, 0);
      if (r == 0 && can_skip_iocp_on_success)
        return true;
      if (r == -1) {
