...
public:
  ...
  void set_send_timeout(uint32 ms) {
    _s.send_timeout = ms <= 65535 ? (uint16)ms : 65535;
  }
  void set_recv_timeout(uint32 ms) {
    _s.recv_timeout = ms <= 65535 ? (uint16)ms : 65535;
  }
  ...
  void set_shut_read() {
    if (atomic_or(&_s.flags, f_shut_read) & f_shut_write)
      this->clear();
  }

  void set_shut_write() {
    if (atomic_or(&_s.flags, f_shut_write) & f_shut_read)
      this->clear();
  }
...
public:
  ...
  HookCtx &get_hook_ctx(sock_t s) { return _tb[(size_t)s]; }
...
void WINAPI hook_Sleep(DWORD a0) {
  ...
  if (!co::gSched || FLG_disable_hook_sleep)
    return CO_RAW_API(Sleep)(a0);
  co::gSched->sleep(a0);
}
...
SOCKET WINAPI hook_socket(int a0, int a1, int a2) {
  ...
  if (s != INVALID_SOCKET) {
    if (a1 != SOCK_STREAM)
      gHook().get_hook_ctx(s).set_non_sock_stream();
  }
...
SOCKET WINAPI hook_WSASocketA(int a0, int a1, int a2, LPWSAPROTOCOL_INFOA a3,
  ...
  if (s != INVALID_SOCKET) {
    ...
    if (!(a5 & WSA_FLAG_OVERLAPPED))
      ctx.set_non_overlapped();
    if (a1 != SOCK_STREAM)
      ctx.set_non_sock_stream();
  }
  HOOKLOG << "hook_WSASocketA, sock: " << s
          << ", overlapped: " << (a5 & WSA_FLAG_OVERLAPPED);
  return s;
}
...
SOCKET WINAPI hook_WSASocketW(int a0, int a1, int a2, LPWSAPROTOCOL_INFOW a3,
  ...
  HOOKLOG << "hook_WSASocketW, sock: " << s
          << ", overlapped: " << (a5 & WSA_FLAG_OVERLAPPED);
  return s;
}
...
int WINAPI hook_closesocket(SOCKET s) {
  if (s == INVALID_SOCKET)
    return CO_RAW_API(closesocket)(s);
...
int WINAPI hook_shutdown(SOCKET a0, int a1) {
  ...
  if (a0 == INVALID_SOCKET)
    return CO_RAW_API(shutdown)(a0, a1);
...
int WINAPI hook_ioctlsocket(SOCKET a0, long a1, u_long *a2) {
  ...
  if (r == 0 && a1 == FIONBIO) {
    HOOKLOG << "hook_ioctlsocket, sock: " << a0
            << ", non_blocking: " << !!(*a2);
...
int WINAPI hook_WSAIoctl(SOCKET a0, DWORD a1, LPVOID a2, DWORD a3, LPVOID a4,
  ...
  if (!co::gSched || a0 == INVALID_SOCKET || a1 == FIONREAD ||
      a1 == SIOCATMARK) {
  ...
  while (true) {
    ...
    if (WSAGetLastError() != WSAEWOULDBLOCK)
      break;
    WLOG_FIRST_N(8) << "hook_WSAIoctl, sock: " << a0 << ", code: " << a1
                    << ", EWOULDBLOCK..";
    co::gSched->sleep(16);
  }
...
SOCKET WINAPI hook_accept(SOCKET a0, sockaddr *a1, int *a2) {
  ...
  if (!co::gSched || a0 == INVALID_SOCKET)
    return CO_RAW_API(accept)(a0, a1, a2);

  auto &ctx = gHook().get_hook_ctx(a0);
  if (ctx.is_non_blocking())
    return CO_RAW_API(accept)(a0, a1, a2);
  if (ctx.is_overlapped())
    return co::accept(a0, a1, a2);
...
SOCKET WINAPI hook_WSAAccept(SOCKET a0, sockaddr *a1, LPINT a2,
  ...
  if (!co::gSched || a0 == INVALID_SOCKET)
    return CO_RAW_API(WSAAccept)(a0, a1, a2, a3, a4);
  ...
  if (a3) {
    ...
    WLOG_FIRST_N(4)
        << "performance warning: WSAAccept on non-overlapped socket: " << a0;
  }
...
int WINAPI hook_connect(SOCKET a0, CONST sockaddr *a1, int a2) {
  ...
  if (!co::gSched || a0 == INVALID_SOCKET)
    return CO_RAW_API(connect)(a0, a1, a2);

  auto &ctx = gHook().get_hook_ctx(a0);
  if (ctx.is_non_blocking())
    return CO_RAW_API(connect)(a0, a1, a2);
  if (ctx.is_overlapped())
    return co::connect(a0, a1, a2, ctx.send_timeout());

  {
    WLOG_FIRST_N(4) << "performance warning: connect on non-overlapped socket: "
                    << a0;
    set_non_blocking(a0, 1);
...
int WINAPI hook_WSAConnect(SOCKET a0, sockaddr *a1, int a2, LPWSABUF a3,
  ...
  if (!co::gSched || a0 == INVALID_SOCKET)
    return CO_RAW_API(WSAConnect)(a0, a1, a2, a3, a4, a5, a6);
...
static void clean_wsabufs(LPWSABUF x, LPWSABUF p, DWORD n, int do_memcpy) {
  for (DWORD i = 0; i < n; ++i) {
    if (x[i].buf != p[i].buf) {
      if (do_memcpy)
        memcpy(p[i].buf, x[i].buf, p[i].len);
      free(x[i].buf);
    }
  }
  free(x);
...
int WINAPI hook_recv(SOCKET a0, char *a1, int a2, int a3) {
  ...
  if (!co::gSched || a0 == INVALID_SOCKET || a2 < 0)
    return CO_RAW_API(recv)(a0, a1, a2, a3);
  ...
  if (ctx.is_overlapped()) {
    ...
    int r =
        CO_RAW_API(WSARecv)(a0, &ev->buf, 1, &ev->n, &ev->flags, &ev->ol, 0);
    if (r == 0) {
      if (!co::can_skip_iocp_on_success)
        ev.wait();
    ...
    WLOG_FIRST_N(8) << "performance warning: recv on non-overlapped socket: "
                    << a0;
    do_hard_hook(ctx, a0, ctx.recv_timeout(), 16,
                 CO_RAW_API(recv)(a0, a1, a2, a3));
  }
}
...
int WINAPI hook_WSARecv(SOCKET a0, LPWSABUF a1, DWORD a2, LPDWORD a3,
  ...
  if (!co::gSched || a0 == INVALID_SOCKET)
    return CO_RAW_API(WSARecv)(a0, a1, a2, a3, a4, a5, a6);
  ...
  if (ctx.is_overlapped()) {
    ...
    WLOG_FIRST_N(8) << "performance warning: WSARecv on non-overlapped socket: "
                    << a0;
    do_hard_hook(ctx, a0, ctx.recv_timeout(), 16,
                 CO_RAW_API(WSARecv)(a0, a1, a2, a3, a4, a5, a6));
  }
}
...
int WINAPI hook_recvfrom(SOCKET a0, char *a1, int a2, int a3, sockaddr *a4,
  ...
  if (!co::gSched || a0 == INVALID_SOCKET || a2 < 0)
    return CO_RAW_API(recvfrom)(a0, a1, a2, a3, a4, a5);
  ...
  if (ctx.is_overlapped()) {
    ...
    if (N > 0) {
      ...
      *(int *)s = sizeof(SOCKADDR_STORAGE);
      r = CO_RAW_API(WSARecvFrom)(a0, &ev->buf, 1, &ev->n, &ev->flags,
                                  (sockaddr *)(s + 8), (int *)s, &ev->ol, 0);
    } else {
      r = CO_RAW_API(WSARecvFrom)(a0, &ev->buf, 1, &ev->n, &ev->flags, 0, 0,
                                  &ev->ol, 0);
    }
    ...
    WLOG_FIRST_N(8)
        << "performance warning: recvfrom on non-overlapped socket: " << a0;
    do_hard_hook(ctx, a0, ctx.recv_timeout(), 16,
                 CO_RAW_API(recvfrom)(a0, a1, a2, a3, a4, a5));
  }
}
...
int WINAPI hook_WSARecvFrom(SOCKET a0, LPWSABUF a1, DWORD a2, LPDWORD a3,
  ...
  if (!co::gSched || a0 == INVALID_SOCKET)
    return CO_RAW_API(WSARecvFrom)(a0, a1, a2, a3, a4, a5, a6, a7, a8);
  ...
  if (ctx.is_overlapped()) {
    ...
    if (N > 0) {
      s = ev->s;
      *(int *)s = sizeof(SOCKADDR_STORAGE);
      r = CO_RAW_API(WSARecvFrom)(a0, x, a2, &ev->n, &ev->flags,
                                  (sockaddr *)(s + 8), (int *)s, &ev->ol, 0);
    } else {
      r = CO_RAW_API(WSARecvFrom)(a0, x, a2, &ev->n, &ev->flags, 0, 0, &ev->ol,
                                  0);
    }
    ...
    do_hard_hook(ctx, a0, ctx.recv_timeout(), 16,
                 CO_RAW_API(WSARecvFrom)(a0, a1, a2, a3, a4, a5, a6, a7, a8));
  }
}
...
int WINAPI hook_send(SOCKET a0, CONST char *a1, int a2, int a3) {
  ...
  if (!co::gSched || a0 == INVALID_SOCKET || a2 < 0)
    return CO_RAW_API(send)(a0, a1, a2, a3);
  ...
  if (ctx.is_overlapped()) {
    ...
    WLOG_FIRST_N(8) << "performance warning: send on non-overlapped socket: "
                    << a0;
    do_hard_hook(ctx, a0, ctx.send_timeout(), 16,
                 CO_RAW_API(send)(a0, a1, a2, a3));
  }
}
...
int WINAPI hook_WSASend(SOCKET a0, LPWSABUF a1, DWORD a2, LPDWORD a3, DWORD a4,
  ...
  if (!co::gSched || a0 == INVALID_SOCKET)
    return CO_RAW_API(WSASend)(a0, a1, a2, a3, a4, a5, a6);
  ...
  if (ctx.is_overlapped()) {
    ...
    *a3 = (r == 0 ? ev->n : 0);
    ...
    WLOG_FIRST_N(8) << "performance warning: WSASend on non-overlapped socket: "
                    << a0;
    do_hard_hook(ctx, a0, ctx.send_timeout(), 16,
                 CO_RAW_API(WSASend)(a0, a1, a2, a3, a4, a5, a6));
  }
}
...
int WINAPI hook_sendto(SOCKET a0, CONST char *a1, int a2, int a3,
  ...
  if (!co::gSched || a0 == INVALID_SOCKET || a2 < 0)
    return CO_RAW_API(sendto)(a0, a1, a2, a3, a4, a5);
  ...
  if (ctx.is_overlapped()) {
    ...
    int r =
        CO_RAW_API(WSASendTo)(a0, &ev->buf, 1, &ev->n, a3, a4, a5, &ev->ol, 0);
    if (r == 0) {
      if (!co::can_skip_iocp_on_success)
        ev.wait();
    } else if (co::error() == WSA_IO_PENDING) {
    ...
    WLOG_FIRST_N(8) << "performance warning: sendto on non-overlapped socket: "
                    << a0;
    do_hard_hook(ctx, a0, ctx.send_timeout(), 16,
                 CO_RAW_API(sendto)(a0, a1, a2, a3, a4, a5));
  }
}
...
int WINAPI hook_WSASendTo(SOCKET a0, LPWSABUF a1, DWORD a2, LPDWORD a3,
  ...
  if (!co::gSched || a0 == INVALID_SOCKET)
    return CO_RAW_API(WSASendTo)(a0, a1, a2, a3, a4, a5, a6, a7, a8);
  ...
  if (ctx.is_overlapped()) {
    ...
    *a3 = (r == 0 ? ev->n : 0);
    ...
    WLOG_FIRST_N(8)
        << "performance warning: WSASendTo on non-overlapped socket: " << a0;
    do_hard_hook(ctx, a0, ctx.send_timeout(), 16,
                 CO_RAW_API(WSASendTo)(a0, a1, a2, a3, a4, a5, a6, a7, a8));
  }
}
...
static void clean_wsamsg(LPWSAMSG x, LPWSAMSG p, int do_memcpy) {
  ...
  if (x->Control.buf != p->Control.buf) {
    if (do_memcpy)
      memcpy(p->Control.buf, x->Control.buf, p->Control.len);
    free(x->Control.buf);
  }
...
int WINAPI hook_WSARecvMsg(SOCKET a0, LPWSAMSG a1, LPDWORD a2,
  ...
  if (ctx.is_overlapped()) {
    ...
    WLOG_FIRST_N(8)
        << "performance warning: WSARecvMsg on non-overlapped socket: " << a0;
    do_hard_hook(ctx, a0, ctx.recv_timeout(), 16,
                 CO_RAW_API(WSARecvMsg)(a0, a1, a2, a3, a4));
  }
}
...
int WINAPI hook_WSASendMsg(SOCKET a0, LPWSAMSG a1, DWORD a2, LPDWORD a3,
  ...
  if (ctx.is_overlapped()) {
    ...
    WLOG_FIRST_N(8)
        << "performance warning: WSARecvMsg on non-overlapped socket: " << a0;
    do_hard_hook(ctx, a0, ctx.send_timeout(), 16,
                 CO_RAW_API(WSASendMsg)(a0, a1, a2, a3, a4, a5));
  }
}

int WINAPI hook_select(int a0, fd_set *a1, fd_set *a2, fd_set *a3,
                       const timeval *a4) {
  if (!co::gSched || (!a1 && !a2 && !a3))
    return CO_RAW_API(select)(a0, a1, a2, a3, a4);
...
int WINAPI hook_WSAPoll(LPWSAPOLLFD a0, ULONG a1, INT a2) {
  ...
  if (!co::gSched || a2 == 0)
    return CO_RAW_API(WSAPoll)(a0, a1, a2);
...
DWORD WINAPI hook_WSAWaitForMultipleEvents(DWORD a0, CONST HANDLE *a1, BOOL a2,
  ...
  if (!co::gSched || a3 == 0)
    return CO_RAW_API(WSAWaitForMultipleEvents)(a0, a1, a2, a3, a4);
...
BOOL WINAPI hook_GetQueuedCompletionStatus(HANDLE a0, LPDWORD a1, PULONG_PTR a2,
  ...
  if (!co::gSched)
    return CO_RAW_API(GetQueuedCompletionStatus)(a0, a1, a2, a3, a4);
...
BOOL WINAPI hook_GetQueuedCompletionStatusEx(HANDLE a0, LPOVERLAPPED_ENTRY a1,
                                             ...
                                             BOOL a5) {
  HOOKLOG << "hook_GetQueuedCompletionStatusEx, handle: " << a0
          << ", ms: " << a4;
  if (!co::gSched)
    return CO_RAW_API(GetQueuedCompletionStatusEx)(a0, a1, a2, a3, a4, a5);
...
#define attach_hook(x)                                                         \
  detour_attach(&(PVOID &)CO_RAW_API(x), (PVOID)hook_##x, #x)
#define detach_hook(x)                                                         \
  detour_detach(&(PVOID &)CO_RAW_API(x), (PVOID)hook_##x, #x)
...
void disable_hook_sleep() { atomic_swap(&FLG_disable_hook_sleep, true); }

void enable_hook_sleep() { atomic_swap(&FLG_disable_hook_sleep, false); }
