...
public:
  ...
  HookCtx &get_hook_ctx(int s) { return _tb[s]; }
...
#define init_hook(f)                                                           \
  if (!CO_RAW_API(f))                                                          \
  atomic_set(&CO_RAW_API(f), dlsym(RTLD_NEXT, #f))
...
#endif
    HOOKLOG << "hook socket, sock: " << s
            << ", non_block: " << ctx.is_non_blocking();
  }
  return s;
}
...
int pipe2(int pipefd[2], int flags) {
  ...
  if (r == 0) {
    ...
    HOOKLOG << "hook pipe2, fd: " << pipefd[0] << ", " << pipefd[1]
...
int fcntl(int fd, int cmd, ... /* arg */) {
  ...
  if (cmd == F_SETFL) {
    ...
    if (r != -1) {
      ...
      HOOKLOG << "hook fcntl F_SETFL, fd: " << fd
              << ", non_block: " << (v & O_NONBLOCK);
    }
...
int ioctl(int fd, co::ioctl_param<ioctl_fp_t>::type request, ...) {
  ...
  if (r != -1) {
    if (request == FIONBIO && ctx.is_sock_or_pipe()) {
      ...
      HOOKLOG << "hook ioctl FIONBIO, fd: " << fd
              << ", non_block: " << *(int *)arg;
    }
  }
...
int dup2(int oldfd, int newfd) {
  ...
  if (oldfd < 0 || newfd < 0 || oldfd == newfd)
    return CO_RAW_API(dup2)(oldfd, newfd);
  ...
  HOOKLOG << "hook dup2, oldfd: " << oldfd << ", newfd: " << newfd
          << ", r: " << r;
  return r;
}
...
int dup3(int oldfd, int newfd, int flags) {
  ...
  if (oldfd < 0 || newfd < 0 || oldfd == newfd)
    return CO_RAW_API(dup3)(oldfd, newfd, flags);
...
int setsockopt(int sockfd, int level, int optname, const void *optval,
  ...
  if (r == 0 && level == SOL_SOCKET &&
    ...
    if (optname == SO_RCVTIMEO) {
      HOOKLOG << "hook_setsockopt, sock: " << sockfd
              << ", recv timeout: " << ms;
...
int connect(int fd, const struct sockaddr *addr, socklen_t addrlen) {
  ...
  if (!co::gSched || fd < 0)
    return CO_RAW_API(connect)(fd, addr, addrlen);
  ...
  if (r == -1 && errno == ETIMEDOUT)
    errno = EINPROGRESS; // set errno to EINPROGRESS
  return r;
}
...
int accept(int fd, struct sockaddr *addr, socklen_t *addrlen) {
  ...
  if (!co::gSched || fd < 0)
    return CO_RAW_API(accept)(fd, addr, addrlen);
...
ssize_t read(int fd, void *buf, size_t count) {
  ...
  if (!co::gSched || fd < 0)
    return CO_RAW_API(read)(fd, buf, count);

  auto &ctx = gHook().get_hook_ctx(fd);
  if (!ctx.is_sock_or_pipe() || ctx.is_non_blocking())
    return CO_RAW_API(read)(fd, buf, count);
...
ssize_t readv(int fd, const struct iovec *iov, int iovcnt) {
  ...
  if (!co::gSched || fd < 0)
    return CO_RAW_API(readv)(fd, iov, iovcnt);
...
ssize_t recv(int fd, void *buf, size_t len, int flags) {
  ...
  if (!co::gSched || fd < 0)
    return CO_RAW_API(recv)(fd, buf, len, flags);
...
ssize_t recvfrom(int fd, void *buf, size_t len, int flags,
  ...
  if (!co::gSched || fd < 0)
    return CO_RAW_API(recvfrom)(fd, buf, len, flags, addr, addrlen);
  ...
  co::IoEvent ev(fd, co::ev_read);
  do_hook(CO_RAW_API(recvfrom)(fd, buf, len, flags, addr, addrlen), ev,
          ctx.recv_timeout());
}
...
ssize_t recvmsg(int fd, struct msghdr *msg, int flags) {
  ...
  if (!co::gSched || fd < 0)
    return CO_RAW_API(recvmsg)(fd, msg, flags);
...
ssize_t write(int fd, const void *buf, size_t count) {
  ...
  if (!co::gSched || fd < 0)
    return CO_RAW_API(write)(fd, buf, count);
...
ssize_t writev(int fd, const struct iovec *iov, int iovcnt) {
  ...
  if (!co::gSched || fd < 0)
    return CO_RAW_API(writev)(fd, iov, iovcnt);
...
ssize_t send(int fd, const void *buf, size_t len, int flags) {
  ...
  if (!co::gSched || fd < 0)
    return CO_RAW_API(send)(fd, buf, len, flags);
...
ssize_t sendto(int fd, const void *buf, size_t len, int flags,
  ...
  if (!co::gSched || fd < 0)
    return CO_RAW_API(sendto)(fd, buf, len, flags, addr, addrlen);
  ...
  co::IoEvent ev(fd, co::ev_write);
  do_hook(CO_RAW_API(sendto)(fd, buf, len, flags, addr, addrlen), ev,
          ctx.send_timeout());
}
...
ssize_t sendmsg(int fd, const struct msghdr *msg, int flags) {
  ...
  if (!co::gSched || fd < 0)
    return CO_RAW_API(sendmsg)(fd, msg, flags);
...
int poll(struct pollfd *fds, nfds_t nfds, int ms) {
  ...
  if (!co::gSched || ms == 0)
    return CO_RAW_API(poll)(fds, nfds, ms);
  ...
  do {
    if (nfds == 1) {
      ...
      if (fds[0].events == POLLIN) {
        if (!co::gSched->add_io_event(fd, co::ev_read))
          break;
      } else if (fds[0].events == POLLOUT) {
        if (!co::gSched->add_io_event(fd, co::ev_write))
          break;
      } else {
        break;
      }
      ...
      if (ms > 0)
        co::gSched->add_timer(ms);
      co::gSched->yield();
      co::gSched->del_io_event(fd);
      if (ms > 0 && co::gSched->timeout())
        return 0;
...
int select(int nfds, fd_set *r, fd_set *w, fd_set *e, struct timeval *tv) {
  init_hook(select);
  if (!co::gSched)
    return CO_RAW_API(select)(nfds, r, w, e, tv);
...
int nanosleep(const struct timespec *req, struct timespec *rem) {
  init_hook(nanosleep);
  if (!co::gSched || FLG_disable_hook_sleep)
    return CO_RAW_API(nanosleep)(req, rem);
...
int epoll_wait(int epfd, struct epoll_event *events, int n, int ms) {
  ...
  if (!co::gSched || epfd < 0 || ms == 0)
    return CO_RAW_API(epoll_wait)(epfd, events, n, ms);
...
int accept4(int fd, struct sockaddr *addr, socklen_t *addrlen, int flags) {
  ...
  if (!co::gSched || fd < 0)
    return CO_RAW_API(accept4)(fd, addr, addrlen, flags);
...
int gethostbyaddr_r(const void *addr, socklen_t addrlen, int type,
  ...
  if (!co::gSched)
    return CO_RAW_API(gethostbyaddr_r)(addr, addrlen, type, ret, buf, len, res,
                                       err);
  co::MutexGuard g(gDnsMutex_t());
  return CO_RAW_API(gethostbyaddr_r)(addr, addrlen, type, ret, buf, len, res,
                                     err);
...
struct hostent *gethostbyname2(const char *name, int af) {
  ...
  if (!co::gSched || !name)
    return CO_RAW_API(gethostbyname2)(name, af);
...
int kevent(int kq, const struct kevent *c, int nc, struct kevent *e, int ne,
  ...
  if (!co::gSched || c || kq < 0)
    return CO_RAW_API(kevent)(kq, c, nc, e, ne, ts);
...
struct hostent *gethostbyname(const char *name) {
  ...
  if (!co::gSched)
    return CO_RAW_API(gethostbyname)(name);
...
struct hostent *gethostbyaddr(const void *addr, socklen_t len, int type) {
  ...
  if (!co::gSched)
    return CO_RAW_API(gethostbyaddr)(addr, len, type);
...
void init() {
  (void)gHook();
  if (CO_RAW_API(close) == 0) {
    (void)::close(-1);
  }
  if (CO_RAW_API(read) == 0) {
    auto r = ::read(-1, 0, 0);
    (void)r;
  }
  ...
  if (CO_RAW_API(pipe) == 0) {
    auto r = ::pipe((int *)0);
    (void)r;
  }
  if (CO_RAW_API(fcntl) == 0) {
    (void)::fcntl(-1, 0);
  }
...
#ifdef __linux__
  if (CO_RAW_API(epoll_wait) == 0)
    ::epoll_wait(-1, 0, 0, 0);
  CHECK(CO_RAW_API(epoll_wait) != 0);
#else
  if (CO_RAW_API(kevent) == 0)
    ::kevent(-1, 0, 0, 0, 0, 0);
