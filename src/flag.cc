...
Flag::Flag(char type, const char *name, const char *value, const char *help,
           const char *file, int line, void *addr)
    : type(type), name(name), value(value), help(help), file(file), line(line),
      lv(10), addr(addr) {
...
template <typename T> fastring int_to_string(T t) {
  if ((0 <= t && t <= 8192) || (t < 0 && t >= -8192))
    return str::from(t);
...
void add_flag(char type, const char *name, const char *value, const char *help,
              const char *file, int line, void *addr) {
  auto r = gFlags().insert(std::make_pair(
      fastring(name), Flag(type, name, value, help, file, line, addr)));

  if (!r.second) {
    COUT << "multiple definitions of flag: " << name << ", from "
         << r.first->second.file << " and " << file;
    exit(0);
  }
}
...
fastring format_str(const fastring &s) {
  ...
  size_t py = s.find('\'');
  size_t pz = s.find('`');
  if (px == s.npos && py == s.npos && pz == s.npos)
    return s;
  if (px == s.npos)
    return fastring(s.size() + 3).append('"').append(s).append('"');
  if (py == s.npos)
    return fastring(s.size() + 3).append('\'').append(s).append('\'');
  if (pz == s.npos)
    return fastring(s.size() + 3).append('`').append(s).append('`');
...
void mkconf(const fastring &exe) {
  ...
  for (auto it = gFlags().begin(); it != gFlags().end(); ++it) {
    Flag *f = &it->second;
    if (f->help[0] == '.' || f->help[0] == '\0')
      continue; // ignore hidden flags.
...
std::vector<fastring> analyze(const std::vector<fastring> &args,
...
void parse_config(const fastring &config) {
  ...
  char sep = '\n';
  if (data.find('\n') == data.npos && data.find('\r') != data.npos)
    sep = '\r';
  ...
  for (size_t i = 0; i < lines.size();) {
    ...
    if (s.empty() || s[0] == '#' || s.starts_with("//"))
      continue;

    size_t p = s.find('=');
    if (p == 0 || p == s.npos) {
      COUT << "invalid config: " << s << ", at " << config << ':'
           << (lineno + 1);
      exit(0);
    }
...
void init(const fastring &path) { xx::parse_config(path); }
