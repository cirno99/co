...
inline const char *Parser::parse_key(const char *b, const char *e,
                                     uint32 &index) {
  if (*b++ != '"')
    return 0;
  const char *p = (const char *)memchr(b, '"', e - b);
  if (p)
    index = _root->_make_key(b, p - b);
  return p;
}

inline const char *Parser::parse_false(const char *b, const char *e,
                                       uint32 &index) {
...
inline const char *Parser::parse_true(const char *b, const char *e,
                                      uint32 &index) {
...
inline const char *Parser::parse_null(const char *b, const char *e,
                                      uint32 &index) {
...
inline const char *find_quote(const char *b, const char *e) {
  return (const char *)memchr(b, '"', e - b);
}
inline const char *find_backslash(const char *b, const char *e) {
  return (const char *)memchr(b, '\\', e - b);
}

const char *Parser::parse_string(const char *b, const char *e, uint32 &index) {
  const char *p, *q;
  if ((p = find_quote(++b, e)) == 0)
    return 0;
...
const char *Parser::parse_number(const char *b, const char *e, uint32 &index) {
  ...
  if (*p == '.') {
    if (++p == e || !is_digit(*p))
      return 0; // must be a digit after the point
    is_double = true;
    while (++p < e && is_digit(*p))
      ;
    if (p == e)
...
digit_end : {
...
}
  {
    ...
    int m = memcmp(
        b, (*b != '-' ? "18446744073709551615" : "-9223372036854775808"), 20);
    if (m < 0)
      goto to_int;
    if (m > 0)
      goto to_dbl;
...
fastream &Json::_Json2pretty(fastream &fs, int indent, int n,
                             uint32 index) const {
  _Header *h = (_Header *)_p8(index);
  if (h->type == kObject) {
    fs << '{';
