...
TestSaver::TestSaver(Test *test) { gTests().push_back(test); }
...
void run_all_tests() {
  ...
  for (size_t i = 0; i < tests.size(); ++i) {
    ...
    if (FLG_a || test->enabled()) {
      ...
      t.restart();
      test->run();
      cout << "< test " << test->name() << " done in " << t.us() << " us"
           << endl;
      ++n;
    }
  ...
  if (failed.empty()) {
    if (n > 0) {
      cout << green << "\nCongratulations! All tests passed!" << default_color
           << endl;
    } else {
    ...
    cout << red << "\nAha! " << ncasesfailed << " case"
         << (ncasesfailed > 1 ? "s" : "");
    cout << " from " << ntestsfailed << " test"
         << (ncasesfailed > 1 ? "s" : "");
...
std::ostream &operator<<(std::ostream &os, const Color &color) {
  if (ansi_color_seq_enabled()) {
    ...
    if (std_handle())
      SetConsoleTextAttribute(std_handle(), (WORD)color.i);
    return os;
  }
}
#endif
