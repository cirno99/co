 ...
 *   - The callback has 2 parameters, a pointer to the log buffer and its
 * length. The buffer may contain more than one logs.
 */
 ...
 */
__codec void
set_single_write_cb(const std::function<void(const void *, size_t)> &cb);
...
// LOG << "hello world " << 23;
// WLOG_IF(1 + 1 == 2) << "xx";
#define _CO_LOG_STREAM(lv)                                                     \
  log::xx::LevelLogSaver(__FILE__, sizeof(__FILE__) - 1, __LINE__, lv).stream()
#define _CO_FLOG_STREAM                                                        \
  log::xx::FatalLogSaver(__FILE__, sizeof(__FILE__) - 1, __LINE__).stream()
#define DLOG                                                                   \
...
#define _co_log_counter_name                                                   \
  _co_log_counter_make_name(_co_log_counter_, __LINE__)

#define _CO_LOG_EVERY_N(n, what)                                               \
  static unsigned int _co_log_counter_name = 0;                                \
  if (atomic_fetch_inc(&_co_log_counter_name) % (n) == 0)                      \
  what

#define _CO_LOG_FIRST_N(n, what)                                               \
  static int _co_log_counter_name = 0;                                         \
  if (_co_log_counter_name < (n) &&                                            \
      atomic_fetch_inc(&_co_log_counter_name) < (n))                           \
  what
