...
inline bool mutex_try_lock(mutex_t *m) {
...
inline void cond_wait(cond_t *c, mutex_t *m) {
  SleepConditionVariableCS(c, m, INFINITE);
}
inline bool cond_wait(cond_t *c, mutex_t *m, uint32 ms) {
...
inline void thread_start(thread_t *t, thread_fun_t f, void *p) {
...
inline void tls_destroy(tls_t k) {
...
class __codec Mutex {
public:
  Mutex() { co::xx::mutex_init(&_mutex); }

  ~Mutex() { co::xx::mutex_destroy(&_mutex); }

  void lock() { co::xx::mutex_lock(&_mutex); }

  void unlock() { co::xx::mutex_unlock(&_mutex); }

  bool try_lock() { return co::xx::mutex_try_lock(&_mutex); }

  co::xx::mutex_t *mutex() { return &_mutex; }
...
public:
  ...
  ~SyncEvent() { co::xx::cond_destroy(&_cond); }
...
public:
  ...
  template <typename F>
  explicit Thread(F &&f) : Thread(co::new_closure(std::forward<F>(f))) {}
...
inline uint32 current_thread_id() { return co::thread_id(); }
...
template <typename T, typename D = std::default_delete<T>> class thread_ptr {
public:
  thread_ptr() { co::xx::tls_init(&_key); }

  ~thread_ptr() { co::xx::tls_destroy(_key); }

  T *get() const { return (T *)co::xx::tls_get(_key); }
