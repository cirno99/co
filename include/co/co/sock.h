...
inline const char *strerror() { return co::strerror(co::error()); }
 ...
 *   - Users can call co::error() to get the errno, and call co::timeout() to
 ...
 *   - The errno will be set to ETIMEDOUT on timeout, call co::error() to get
 * the errno, or simply call co::timeout() to check whether it has timed out.
 *
 * @param fd   a non-blocking (also overlapped on windows) socket.
 ...
 * returned if fd is a stream socket and the peer has closed the connection.
 */
__codec int recvfrom(sock_t fd, void *buf, int n, void *src_addr, int *addrlen,
                     int ms = -1);
 ...
 */
__codec int sendto(sock_t fd, const void *buf, int n, const void *dst_addr,
                   int addrlen, int ms = -1);
