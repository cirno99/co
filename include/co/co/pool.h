...
public:
  ...
  Pool(const Pool &p) : _p(p._p) { atomic_inc(_p); }
 ...
 *   - operator->() is overloaded, so it has a behavior similar to
 * std::unique_ptr.
 *
 *   - usage:
 *     struct T { void hello(); };
 ...
 *     g->hello();
 */
template <typename T, typename D = std::default_delete<T>> class PoolGuard {
public:
  explicit PoolGuard(const Pool &pool) : _pool(pool) { _p = (T *)_pool.pop(); }

  explicit PoolGuard(const Pool *pool) : _pool(*pool) { _p = (T *)_pool.pop(); }

  ~PoolGuard() { _pool.push(_p); }
   ...
   *
   * @param p  a pointer to an object of class T, it MUST be created with
   * operator new. default: NULL.
   */
  void reset(T *p = 0) {
    if (_p != p) {
      if (_p)
