...
public:
  ...
  Mutex(const Mutex &m) : _p(m._p) { atomic_inc(_p); }
   ...
   *   - If no coroutine holds the lock, the calling coroutine will get the
   * lock.
   *
   * @return  true if the lock was acquired by the calling coroutine, otherwise
   * false
   */
  bool try_lock() const;
