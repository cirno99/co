...
public:
  ...
  PerIoInfo *operator->() const { return _info; }
   ...
   *     any error occured.
   *   - If the second constructor was used, users MUST firstly create an
   * IoEvent, and call WSARecv, WSASend or other APIs to post an IO operation to
   * IOCP, and then call wait() to wait for the operation to be done.
   *
...
public:
   ...
   * @param ev  the IO event, either ev_read or ev_write.
   */
  IoEvent(sock_t fd, io_event_t ev) : _fd(fd), _ev(ev), _has_ev(false) {}
  ...
  /**
   * wait for an IO event on a socket
   *   - wait() blocks until the IO event is present, or timeout, or any error
   * occured.
   *
   * @param ms  timeout in milliseconds, default: -1, never timeout.
   *
