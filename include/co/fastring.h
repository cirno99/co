...
public:
  ...
  fastring() noexcept : fast::stream() {}

  explicit fastring(size_t cap) : fast::stream(cap) {}

  fastring(void *p, size_t size, size_t cap) : fast::stream(p, size, cap) {}
  ...
  fastring &operator+=(signed char c) { return this->append(c); }

  fastring &operator+=(unsigned char c) { return this->append(c); }
...
private:
  ...
  template <typename T> fastring &_out(T &&t, I<0>) {
    return (fastring &)fast::stream::operator<<(std::forward<T>(t));
  }

  // string literal like "hello"
  template <typename T> fastring &_out(T &&t, I<1>) {
    return (fastring &)fast::stream::append(t, sizeof(t) - 1);
  }

  // const char* or char*
  template <typename T> fastring &_out(T &&t, I<2>) { return this->append(t); }

  // fastring, std::string
  template <typename T> fastring &_out(T &&t, I<3>) {
    return this->append((god::const_ref_t<T>)t);
  }

  // other classes, call global `fastring& operator<<(fastring&, const X&)`
  template <typename T> fastring &_out(T &&t, I<4>) {
