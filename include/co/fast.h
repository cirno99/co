...
inline int dtoa(double v, char *buf) { return milo::dtoa(v, buf); }
...
template <typename V> inline int itoa(V v, char *buf, le_32bit<true>) {
  return i32toa((int32)v, buf);
}

template <typename V> inline int itoa(V v, char *buf, le_32bit<false>) {
  return i64toa((int64)v, buf);
}

template <typename V> inline int utoa(V v, char *buf, le_32bit<true>) {
  return u32toa((uint32)v, buf);
}

template <typename V> inline int utoa(V v, char *buf, le_32bit<false>) {
  return u64toa((uint64)v, buf);
}

template <int> int ptoh(const void *p, char *buf);

template <> inline int ptoh<4>(const void *p, char *buf) {
  return u32toh((uint32)(size_t)p, buf);
}

template <> inline int ptoh<8>(const void *p, char *buf) {
  return u64toh((uint64)p, buf);
}
} // namespace xx

// signed integer to ascii string
template <typename V> inline int itoa(V v, char *buf) {
  return xx::itoa(v, buf, xx::le_32bit<sizeof(V) <= sizeof(uint32)>());
}

// unsigned integer to ascii string
template <typename V> inline int utoa(V v, char *buf) {
...
public:
  ...
  explicit stream(size_t cap) : _cap(cap), _size(0), _p((char *)malloc(cap)) {}
...
protected:
  ...
  stream &operator<<(const char *v) { return this->append(v, strlen(v)); }
