 ...
 *     openssl error queue for the current thread as a string, and the error
 * queue will be cleared then.
 *
 * @param s  a pointer to SSL, if s is not NULL, result code of the previous ssl
 * I/O operation will also be checked to obtain more error message.
 *
 ...
 *   - This function will check the result of SSL_get_error(), if
 * SSL_ERROR_SYSCALL or SSL_ERROR_SSL was returned, SSL_shutdown() will not be
 * called.
 *   - See documents here:
 *     https://www.openssl.org/docs/man1.1.0/man3/SSL_get_error.html
 *
 *   - NOTE: Is it necessary to shutdown the SSL connection on TCP? Why not
 * close the underlying TCP connection directly?
 *
 * @param s   a pointer to SSL.
 ...
 *           <=0  an error occured, call ssl::strerror() to get the error
 * message.
 */
 ...
 *   - When an API with a timeout like ssl::recv returns, ssl::timeout() can be
 * called to check whether it has timed out.
 *
