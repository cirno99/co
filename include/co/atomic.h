...
template <> struct I<1> { typedef char type; };

template <> struct I<2> { typedef short type; };

template <> struct I<4> { typedef long type; };

template <> struct I<8> { typedef long long type; };
...
template <typename T> inline T atomic_inc(T *p) {
  typedef xx::I<sizeof(T)> it;
  return (T)xx::atomic_inc(p, it());
}

template <typename T> inline T atomic_dec(T *p) {
  typedef xx::I<sizeof(T)> it;
  return (T)xx::atomic_dec(p, it());
}

template <typename T, typename V> inline T atomic_add(T *p, V v) {
  typedef xx::I<sizeof(T)> it;
  typedef typename it::type type;
  return (T)xx::atomic_add(p, (type)v, it());
}

template <typename T, typename V> inline T atomic_sub(T *p, V v) {
  typedef xx::I<sizeof(T)> it;
  typedef typename it::type type;
  return (T)xx::atomic_sub(p, (type)v, it());
}

template <typename T> inline T atomic_fetch_inc(T *p) {
  typedef xx::I<sizeof(T)> it;
  return (T)xx::atomic_fetch_inc(p, it());
}

template <typename T> inline T atomic_fetch_dec(T *p) {
  typedef xx::I<sizeof(T)> it;
  return (T)xx::atomic_fetch_dec(p, it());
}

template <typename T, typename V> inline T atomic_fetch_add(T *p, V v) {
  typedef xx::I<sizeof(T)> it;
  typedef typename it::type type;
  return (T)xx::atomic_fetch_add(p, (type)v, it());
}

template <typename T, typename V> inline T atomic_fetch_sub(T *p, V v) {
  typedef xx::I<sizeof(T)> it;
  typedef typename it::type type;
  return (T)xx::atomic_fetch_sub(p, (type)v, it());
}

template <typename T, typename V> inline T atomic_or(T *p, V v) {
  typedef xx::I<sizeof(T)> it;
  typedef typename it::type type;
  return (T)xx::atomic_or(p, (type)v, it());
}

template <typename T, typename V> inline T atomic_and(T *p, V v) {
  typedef xx::I<sizeof(T)> it;
  typedef typename it::type type;
  return (T)xx::atomic_and(p, (type)v, it());
}

template <typename T, typename V> inline T atomic_xor(T *p, V v) {
  typedef xx::I<sizeof(T)> it;
  typedef typename it::type type;
  return (T)xx::atomic_xor(p, (type)v, it());
}

template <typename T, typename V> inline T atomic_fetch_or(T *p, V v) {
  typedef xx::I<sizeof(T)> it;
  typedef typename it::type type;
  return (T)xx::atomic_fetch_or(p, (type)v, it());
}

template <typename T, typename V> inline T atomic_fetch_and(T *p, V v) {
  typedef xx::I<sizeof(T)> it;
  typedef typename it::type type;
  return (T)xx::atomic_fetch_and(p, (type)v, it());
}

template <typename T, typename V> inline T atomic_fetch_xor(T *p, V v) {
  typedef xx::I<sizeof(T)> it;
  typedef typename it::type type;
  return (T)xx::atomic_fetch_xor(p, (type)v, it());
}

template <typename T, typename V> inline T atomic_swap(T *p, V v) {
  typedef xx::I<sizeof(T)> it;
  typedef typename it::type type;
...
template <typename T> inline T atomic_get(T *p) {
  return atomic_fetch_xor(p, 0);
}

template <typename T, typename V> inline void atomic_set(T *p, V v) {
  (void)atomic_swap(p, v);
}

template <typename T> inline void atomic_reset(T *p) {
...
inline T atomic_compare_swap(T *p, O o, V v) {
  T x = (T)o;
  __atomic_compare_exchange_n(p, &x, (T)v, false, __ATOMIC_SEQ_CST,
                              __ATOMIC_SEQ_CST);
  return x;
}
