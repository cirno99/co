...
inline xx::E error(const char *s) { return xx::E(s); }

template <typename T, typename X = void> class maybe;
...
public:
  ...
  maybe() : _e(), _has_error(false) {}

  maybe(int) : maybe() {}

  maybe(E &&e) : _e(std::move(e)), _has_error(true) {}

  maybe(maybe &&m) : _e(std::move(m._e)), _has_error(m._has_error) {}
  ...
  const char *error() const { return _e.error(); }
...
public:
  ...
  maybe() : _t(T()), _has_error(false) {}

  maybe(T t) : _t(t), _has_error(false) {}

  maybe(E &&e) : _e(std::move(e)), _has_error(true) {}

  maybe(maybe &&m) : _has_error(m._has_error) {
  ...
  const char *error() const { return this->has_error() ? _e.error() : ""; }

  T value() const { return !this->has_error() ? _t : T(); }
...
// T is reference
template <typename T> class maybe<T, god::enable_if_t<god::is_ref<T>()>> final {
public:
  using E = xx::E;
...
public:
  ...
  maybe(T &&t) : _t(std::move(t)), _has_error(false) {}
