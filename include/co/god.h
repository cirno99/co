 ...
 */
template <typename T> inline bool bytes_eq(const void *p, const void *q) {
...
template <typename T> using remove_arr_t = typename std::remove_extent<T>::type;
...
template <typename T>
using const_ref_t =
    typename std::add_lvalue_reference<add_const_t<remove_ref_t<T>>>::type;
...
template <typename T> inline constexpr bool is_pointer() {
  return std::is_pointer<T>::value || std::is_member_pointer<T>::value ||
         std::is_same<typename std::remove_cv<T>::type,
                      decltype(nullptr)>::value;
}

template <typename T> inline constexpr bool is_ref() {
  return std::is_reference<T>::value;
}

template <typename T> inline constexpr bool is_array() {
  return std::is_array<T>::value;
}

template <typename T> inline constexpr bool is_class() {
  return std::is_class<T>::value;
}

template <typename T> inline constexpr bool is_scalar() {
