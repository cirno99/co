 ...
 *   - If you are using co.dll on windows, it is better to call co::exit()
 * manaully at end of the main function, or the program may hang forever at
 * exit.
 *   - It will also call `log::exit()` after the schedulers are stopped, if you
 * have called `co::init(argc, argv)` or `co::init(config)` before.
 */
__codec void exit();
 ...
 *   - DEF_main can be used to ensure code in main function also runs in
 * coroutine.
 */
 ...
 *
 * @return a pointer to the current scheduler, or NULL if called from a
 ...
 *     xx[co::scheduler_id()] can be used in a coroutine to access the storage
 * for the current scheduler thread.
 *
 ...
 *
 * @return  a non-negative id of the current scheduler, or -1 if the current
 * thread is not a scheduler thread.
 */
 ...
 *
 * @return  a non-negative id of the current coroutine, or -1 if the current
 * thread is not a scheduler thread.
 */
 ...
 * added. When the event is present, the scheduler will resume the coroutine.
 *
 * @param fd  the socket.
 ...
 * coroutine, and then call yield() to suspend the coroutine. When the event is
 * present or the timer expires, the scheduler will resume the coroutine.
 */
__codec void yield();
 ...
 * may call co::timeout() to check whether the API call has timed out.
 *
