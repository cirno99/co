...
__codec void push_failed_msg(const fastring &test_name,
                             const fastring &case_name, const char *file,
                             int line, const fastring &msg);
...
class Case {
public:
  Case(const fastring &name) : _name(name) {
...
#define EXPECT(x)                                                              \
  {                                                                            \
    ...
    if (x) {                                                                   \
      ...
      unitest::push_failed_msg(_name, _current_case->name(), __FILE__,         \
                               __LINE__, _U_s);                                \
    }                                                                          \
  }
...
#define EXPECT_OP(x, y, op, opname)                                            \
  {                                                                            \
    ...
    auto _U_y = (y);                                                           \
                                                                               \
    if (_U_x op _U_y) {                                                        \
      cout << unitest::green << "  EXPECT_" << opname << "(" << #x << ", "     \
           << #y << ") passed";                                                \
      if (strcmp("==", #op) != 0)                                              \
        cout << ": " << _U_x << " vs " << _U_y;                                \
