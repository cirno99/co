...
public:
  ...
  void *at(uint32 index) const { return _h->p + index; }

  void clear() { _h->size = 0; }

  void safe_clear() {
    memset(_h->p, 0, _h->size * N);
    _h->size = 0;
  }
  ...
  size_t memory_size() const { return sizeof(_Header) + _h->cap * N; }
...
struct Stack {
  typedef uint32 T;
  Stack(uint32 n = 256) : cap(n), size(0) { p = (T *)malloc(sizeof(T) * cap); }
  ~Stack() { free(p); }
...
class JAlloc {
public:
  JAlloc() : _fs(256), _stack(512) { _jb.reserve(32); }

  ~JAlloc() {
    for (size_t i = 0; i < _jb.size(); ++i)
      free(_jb[i]);
  }
...
public:
  ...
  public:
    ...
    void operator=(const fastring &x) {
      return _root->_set_string(x.data(), x.size(), _index);
    }
    ...
    void add_member(Key key, S x, size_t n) {
      return _root->_add_member(key, x, n, _index);
    }
    void add_member(Key key, S x) {
    ...
    fastream &pretty(fastream &fs) const {
      return _root->_Json2pretty(fs, 4, 4, _index);
    }
    ...
    fastring pretty(uint32 cap = 256) const {
      fastring s(cap);
      this->pretty(*(fastream *)&s);
      return s;
    }

    class iterator {
    public:
      iterator(Json *root, uint32 q, uint32 type) : _root(root), _q(q), _i(0) {
      ...
      Value operator*() const {
        return Value(_root, ((xx::Queue *)_root->_p8(_q))->p[_i]);
      }
      Key key() const {
        return (Key)_root->_p8(((xx::Queue *)_root->_p8(_q))->p[_i]);
      }
      Value value() const {
        return Value(_root, ((xx::Queue *)_root->_p8(_q))->p[_i + 1]);
      }
  ...
  private:
    ...
    friend class Json;
    Value(Json *root, uint32 index) : _root(root), _index(index) {}
  };
  ...
  ~Json() {
    if (_mem)
      xx::jalloc()->dealloc_jblock(_mem);
  }

  Json(const Json &r) : _mem(xx::jalloc()->alloc_jblock()) {
    _jb.copy_from(r._jb);
  }
  ...
  fastream &pretty(fastream &fs) const {
    return this->_Json2pretty(fs, 4, 4, 0);
  }
  fastring str(uint32 cap = 256) const {
    fastring s(cap);
    this->str(*(fastream *)&s);
...
private:
  ...
  iterator _begin(uint32 index) const {
    _Header *h = (_Header *)_p8(index);
    if (h->type == kNull)
      return iterator((Json *)this, 0, kNull);
    assert(h->type & (kObject | kArray));
    uint32 q = h->index;
    if (q && ((xx::Queue *)_p8(q))->size == 0)
      q = 0;
  ...
  uint32 _make_null() {
    uint32 i = _alloc_header();
    new (_p8(i)) _Header(kNull);
    return i;
  }
  uint32 _make_bool(bool x) {
    uint32 i = _alloc_header();
    (new (_p8(i)) _Header(kBool))->b = x;
    return i;
  }
  uint32 _make_int(int64 x) {
    uint32 i = _alloc_header();
    (new (_p8(i)) _Header(kInt))->i = x;
    return i;
  }
  uint32 _make_double(double x) {
    uint32 i = _alloc_header();
    (new (_p8(i)) _Header(kDouble))->d = x;
    return i;
  }
  ...
  uint32 _make_key(const char *x, size_t n) { return _make_raw_string(x, n); }
  ...
  bool _is_double(uint32 i) const {
    return ((_Header *)_p8(i))->type == kDouble;
  }
  bool _is_string(uint32 i) const {
    return ((_Header *)_p8(i))->type == kString;
  }
  bool _is_array(uint32 i) const { return ((_Header *)_p8(i))->type == kArray; }
  bool _is_object(uint32 i) const {
    return ((_Header *)_p8(i))->type == kObject;
  }

  bool _get_bool(uint32 i) const {
    _Header *h = (_Header *)_p8(i);
    return (h->type == kBool) ? h->b : false;
  }
  int64 _get_int64(uint32 i) const {
    _Header *h = (_Header *)_p8(i);
    return (h->type == kInt) ? h->i : 0;
  }
  double _get_double(uint32 i) const {
    _Header *h = (_Header *)_p8(i);
    return (h->type == kDouble) ? h->d : 0.0;
  }
  S _get_string(uint32 i) const {
    _Header *h = (_Header *)_p8(i);
    return (h->type == kString) ? (S)_p8(h->index) : "";
  }

  void _set_bool(bool x, uint32 i) { (new (_p8(i)) _Header(kBool))->b = x; }
  void _set_int(int64 x, uint32 i) { (new (_p8(i)) _Header(kInt))->i = x; }
  void _set_double(double x, uint32 i) {
    (new (_p8(i)) _Header(kDouble))->d = x;
  }
  ...
  void _set_array(uint32 i) {
    _Header *h = new (_p8(i)) _Header(kArray);
    h->index = 0;
  }
  void _set_object(uint32 i) {
    _Header *h = new (_p8(i)) _Header(kObject);
    h->index = 0;
  }
  ...
  void _add_member(Key key, S x, size_t n, uint32 i) {
  ...
  void _push_back(bool x, uint32 i) {
    return this->_push_back(_make_bool(x), i);
  }
  void _push_back(int64 x, uint32 i) {
    return this->_push_back(_make_int(x), i);
  }
  void _push_back(double x, uint32 i) {
    return this->_push_back(_make_double(x), i);
  }
  void _push_back(S x, size_t n, uint32 i) {
    return this->_push_back(_make_string(x, n), i);
  }
  void _push_null(uint32 i) { return this->_push_back(_make_null(), i); }
  Value _push_array(uint32 cap, uint32 i) {
    uint32 a = _make_array(cap);
    this->_push_back(a, i);
    return Value(this, a);
  }
  Value _push_object(uint32 cap, uint32 i) {
    uint32 o = _make_object(cap);
    this->_push_back(o, i);
    return Value(this, o);
  }
